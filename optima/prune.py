...
from slim_local.slim.sparsegpt import SparseGPT
from slim_local.slim.sparsegpt import Quantizer as SparseGPTQuantizer
from slim_local.slim.layerwrapper import WrappedGPT
from slim_local.slim.data import get_loaders
from slim_local.slim.utils import (
    get_layers_list,
    shift_zeros,
    find_layers,
    prune_nm,
...
from slim_local.slim.quantization.quantization import (
    Quantizer as AutoQuantizer,
    QuantizedMatmul,
)
import tqdm.auto as tqdm
from slim_local.slim.jsq_utils import clip_matrix, generate_ss
from slim_local.slim.smooth import smooth_layer
    ...
    for i in progress_bar:
        ...
        for name in subset:
            ...
            if update_weights:
                ...
                wrapped_layers[name].original_weight = (
                    subset[name].weight.data.clone().detach()
                )
...
def prune_sparsegpt(
    ...
    for i in progress_bar:
        ...
        for name in gpts:
            ...
            if quantize_weight:
                subset[name].register_buffer(
                    "quantization_scaling_factor", 1.0 / gpts[name].quantizer.scale
                )
                if not tiled_weight_quantization:
                    subset[name].scaling_factor = 1.0 / gpts[name].quantizer.scale[0]
                else:
                    subset[name].scaling_factor = None
            gpts[name].free()
...
def prune_and_quantize(
    ...
    if sparsity_ratio == 0.0 or sparsity_type == "dense":
        ...
        if prune_method in ["wanda", "maskllm"]:
            ...
            if scale_important_weights and quantize_weight:
                raise NotImplementedError(
                    "Scaling important weights not implemented for Thanos pruning and "
                    "quantization"
                )
            if lora_rank > 0:
                raise NotImplementedError(
                    "LoRA approximation not implemented for Thanos"
                )
            if quantize_weight:
