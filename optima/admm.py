    ...
    def fasterprune(
        ...
        if iterative_prune == 0:
            if prune_n != 0:
                ...
                #                mask = ((W).abs() > thres)
                topk = torch.topk(
                    W.abs().flatten(), k=int(W.numel() * sparsity), largest=False
                )
        ...
        if iters == 0:
            ...
            self.layer.weight.data = out.reshape(self.layer.weight.shape).to(
        ...
        for itt in range(iters):
            if iterative_prune > 0 and itt < iterative_prune:
                if prune_n != 0:
                    sparsity = 0.5
                    cur_sparsity = (
                        sparsity - sparsity * (1 - (itt + 1) / iterative_prune) ** 3
                    )
                    ...
                    if per_out:
                        thres = (
                            (W + U).abs().sort(dim=0)[0][int(W.shape[0] * cur_sparsity)]
                        )
