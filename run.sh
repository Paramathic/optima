...
for MODEL_SIZE in 7b # 125m 350m 1.3b 2.7b 6.7b 13b #8B #7B #125m # 7b #1.3b #7B #6.7b
do
    for STRUCTURE in 2:4 unstructured 
    do
        for METHOD in wanda 
        do
            for LORA_RANK in 0.1 #0.125 0.25 0.125 0.0625 0.03125 
            do
    #           rm -rf data
                ...
                CUDA_VISIBLE_DEVICES=0 python main_opt.py \
                    ...
                    $ACCELERATE \
                    $TEST_LMHARNESS \
                    --output_csv_path results/uniform_opt_perplexity${FINE_TUNE}.csv \
                    $FINE_TUNE \
                    $EVALUATE_PERPLEXITY \
                    $LOCAL_FILES_ONLY \
                    $QUANTIZE_INPUT \
