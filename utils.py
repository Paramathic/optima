...
def dequantize_tensor(x, alphas, betas, q):
    row_block_size, col_block_size = x.shape[0] // alphas.shape[0], x.shape[1] // alphas.shape[1]
    y = torch.empty_like(x, dtype=torch.float16).cuda()
    grid = lambda meta: (alphas.shape[0], alphas.shape[1])
...
if __name__ == "__main__":
    ...
    for dim2 in [512 * i for i in range(2, 32)]:
        # Allocate memory
        x = torch.randn(dim1, dim2).cuda().half() + (torch.randn(1).cuda().half() * 3.)
        q = 8
        ...
        for i in range(alphas.shape[0]):
            for j in range(alphas.shape[1]):
                ...
                x_dequant[i*block_size1:(i+1)*block_size1, j*block_size2:(j+1)*block_size2] = (block.to(torch.float16) / 127. * alphas[i, j] + betas[i, j])
        ...
        x_dequant_triton = dequantize_tensor(y_triton, alphas, betas, q)
