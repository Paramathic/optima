...
def parse_args():
    ...
    parser.add_argument(
        "--seqlen", type=int, help="Sequence length of the model", default=-1
    )
    parser.add_argument(
        "--max_batch_size", type=int, help="Maximum batch size for inference", default=1
    )
    parser.add_argument(
        "--iterations", type=int, help="Number of iterations to run", default=3
    )
    parser.add_argument(
        "--warmup_iters", type=int, help="Number of warmup iterations", default=2
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--time_prefill", action="store_true", help="Time only the prefill step"
    )
    parser.add_argument(
        "--input_token_step",
        type=int,
        help="Token step size for input generation",
        default=1024,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--compress_model", action="store_true", help="Compress the model"
    )
...
def aggregate_data(csv_path):
    ...
    median_df = (
        df.groupby(["component_type", "component_name", "layer_idx"])["time"]
        .median()
        .reset_index()
    )
    ...
    agg_df = (
        median_df.groupby(["component_type", "component_name"], dropna=False)["time"]
        .sum()
        .reset_index()
    )
    return agg_df
...
if __name__ == "__main__":
    ...
    model_seqlen = (
        model.config.max_position_embeddings if args.seqlen == -1 else args.seqlen
    )

    # Benchmark setup
    token_sizes = []
    for k in [
        args.input_token_step * i
        for i in range(1, model_seqlen // args.input_token_step)
    ]:
    ...
    batch_sizes = [
        2**i
        for i in range(
            int(min(16, math.log2(args.max_batch_size))),
            int(math.log2(args.max_batch_size) + 1),
        )
    ]
    ...
    for token_size in token_sizes:
        ...
        for batch_size in batch_sizes:
            ...
            print(
                f"Input Sequence Length: {token_size}, Output Sequence Length: {max_new_tokens}, "
            ...
            batch_input_ids = torch.stack(extended_chunks[:batch_size], dim=0).to(
                device
            )
            ...
            if not os.path.exists(csv_path):
                with open(csv_path, "w") as f:
                    f.write(
                        "batch_size,token_size,iteration,layer_idx,component_type,component_name,time\n"
                    )
            ...
            with torch.no_grad():
                for it in range(args.iterations):
                    ...
                    with open(csv_path, "a") as f:
                        f.write(
                            f"{batch_size},{token_size},{it},,model_generate,total,{iter_time}\n"
                        )
                        for (
                            layer_idx,
                            comp_type,
                            comp_name,
                        ), t in global_current_data.items():
                            f.write(
                                f"{batch_size},{token_size},{it},{layer_idx},{comp_type},{comp_name},{t}\n"
                            )
